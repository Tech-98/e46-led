...
print("opening seq")
from fullseq import hex_seq_cal_3
...
subprocess.Popen(["python", "-m", "http.server", "8081"])
...
try:
    ledArduino = Interface("COM20")
    print("Connected LED Arduino")

except Exception as e:
    try:
        ledArduino = Interface("/dev/ttyACM1")
        print("Connected LED Arduino")
    except Exception as e:
        try:
            ledArduino = Interface("/dev/ttyACM2")
            print("Connected LED Arduino")
        except Exception as e:
...
try:
    stepperArduino = Inteerface("COM6")
...
def play_slide():
    ...
    while True:
        ...
        if slide_to_play != slide_playing:
            ...
            startTime = round(time.perf_counter(), 3)
            start_frame = timeStamps[slide_to_play] * 30
...
if __name__ == '__main__':
    ...
    # thread.start()
    print("main started")
    socketio.run(app, host="0.0.0.0", port=5678, log_output=True)
